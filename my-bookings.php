...
foreach ($bookings as $id => $booking) {
  ...
  $ticket['direction'] = 2 === count($ticket['flights']) ? 'roundtrip' : 'oneway';
  $ticket['price'] = 2 === count($ticket['flights']) ? $ticket['flights']['a']['price'] + $ticket['flights']['b']['price'] : $ticket['flights']['a']['price'];
...
foreach ($tickets as $orders) {
  foreach ($orders['flights'] as $direction => $info) {
    ...
    if ('roundtrip' == $orders['direction']) {
      $roundtrip_merge = 'a' == $direction ? 'pb-0 mb-0 border-bottom-0' : 'pt-0 pb-3 mb-1 border-top-0';
    } else {
      $roundtrip_merge = null;
    }

    $d_none = 'b' == $direction ? 'd-none' : null;

    $params = [
      '%ROUNDTRIP_MERGE%' => $roundtrip_merge,
      '%ROUNDTRIP_HIDE_ID%' => 'a' == $direction ? 'mb-2 mt-3 d-flex' : 'd-none',
...
function getFlightInfo($id, $direction)
{
  ...
  /** Paginating bookings results */
  $flight = $db->get('bookings b', null, $columns);
