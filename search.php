...
$filterAirlines = $_SESSION['filterAirlines'] ?? [];
...
if (!empty($_SESSION['time_range'])) {
    ...
    $search['date_departure'] = $query_hash['departure_date'] ?? date('Y-m-d');
    $search['date_departure'] = [
        'start' => $search['date_departure'] . $first,
        'end'   => $search['date_departure'] . $last
    ];

    $search['date_returning'] = !empty($query_hash['returning_date'])
        ? ['start' => $query_hash['returning_date'] . $first, 'end' => $query_hash['returning_date'] . $last]
        : null;
}

$search['departure'] = !empty($query_hash['from'])
    ? explode(',', $query_hash['from'])[0]
    : null;
$search['arrival'] = !empty($query_hash['to'])
    ? explode(',', $query_hash['to'])[0]
    : null;
...
$flights_total = !empty($tickets)
    ? count($tickets)
    : $flights_departure['total'];
...
if (0 != $flights_total) {
    ...
    foreach ($tickets as $flight) {
        ...
        $flight['departure']['arrival_time'] = Functions::timeZone(
            $flight['departure']['arrival_time'],
            $flight['departure']['departure_timezone'],
            $flight['departure']['arrival_timezone']
        );

        if (!empty($flight['returning'])) {
            $flight['returning']['arrival_time'] = Functions::timeZone(
                $flight['returning']['arrival_time'],
                $flight['returning']['departure_timezone'],
                $flight['returning']['arrival_timezone']
            );
        }
        ...
        $params = [
            ...
            '%RETURNING_FLIGHT_DB_ID%' => $flight['returning']['id'] ?? null,
