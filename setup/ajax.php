...
$stages = ['createMySQL', 'generateFlights', 'finalStep'];
...
foreach ($stages as $id => $data) {
	...
	$progress['now'] = $progress['now'] >= 100 ? 100 : $progress['now'];
...
function esSend($id, $type, $message, $progress = 0, $inline = false)
{
	...
	$template = !$inline ? '<div class="output mb-0">%s</div>' : '%s';
	$d = [
		'rtype' => $type,
...
function generateFlights()
{
	...
	$flights_count = !empty($_GET['generate_flights']) ? $_GET['generate_flights'] : Config::$setup['flights'];
