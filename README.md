...
(`.github/workflows/release.yml`). The version bump comes from a label on the
pull request, so commit messages stay free-form:

| Label            | Effect on `v2.8.0` |
|------------------|--------------------|
| `release:major`  | `v3.0.0`           |
| `release:minor`  | `v2.9.0`           |
| `release:patch`  | `v2.8.1`           |
| `no-release`     | no release cut     |

Exactly one of those labels is required: `pr-labels.yml` fails a pull request
that has none, or more than one, so the bump is never chosen by accident. (Add
it as a required status check on `develop` for that to block merging.)

Release notes are generated by GitHub from the pull requests included since the
previous tag. The application footer shows the current tag, so a deployed server
needs `git fetch --tags` for it to appear.
